...
function save_player_action( playername, text )
	local time = getRealTime()
		...
		if logged[playername] == 1 then
						...
						if getSpeed(vehicleid) < 1 then
							...
							elseif job_call[playername] == 2 then--сдаем вызов
								if isPointInCircle3D(x,y,z, job_pos[playername][1],job_pos[playername][2],job_pos[playername][3], 40) then
									...
									save_player_action(playername, "[taxi_job_timer] "..playername.." [+"..randomize.."$, "..array_player_2[playername][1].."$]")
					...
					if getElementModel(vehicleid) == 519 then
						if getSpeed(vehicleid) <= 5 then
							...
							if job_call[playername] == 0 then--нету вызова
								...
								sendPlayerMessage(playerid, "Летите в аэропорт "..plane_job[randomize][4], yellow[1], yellow[2], yellow[3])
							...
							elseif job_call[playername] == 1 then--лв
								if isPointInCircle3D(x,y,z, job_pos[playername][1],job_pos[playername][2],job_pos[playername][3], 40) then
									...
									save_player_action(playername, "[pilot_job_timer] "..playername.." [+"..randomize.."$, "..array_player_2[playername][1].."$]")
			...
			elseif (job[playername] == 6) then --работа Угонщик
				...
				if (job_call[playername] == 0) then --нету вызова
					...
					sendPlayerMessage(playerid, "Угоните т/с гос.номер "..getVehiclePlateText(job_vehicleid[playername][1])..", у вас есть "..car_theft_time.." мин", yellow[1], yellow[2], yellow[3])
				...
				elseif (job_call[playername] == 1) then
					...
					if (job_vehicleid[playername][1] == vehicleid) then
						...
						sendPlayerMessage(playerid, "Езжайте в отстойник", yellow[1], yellow[2], yellow[3])
				...
				elseif (job_call[playername] == 2) then--сдаем вызов
					...
					if (isPointInCircle3D(x,y,z, job_pos[playername][1],job_pos[playername][2],job_pos[playername][3], 5.0) and job_vehicleid[playername][1] == vehicleid) then
						...
						if (getSpeed(vehicleid) < 1) then
							...
							save_player_action(playername, "[car_theft_job_timer] "..playername.." [+"..randomize.."$, "..array_player_2[playername][1].."$]")
			...
			elseif job[playername] == 7 then--забойщик скота
				if job_call[playername] == 0 then--нету вызова
					...
					sendPlayerMessage(playerid, "Убейте корову", yellow[1], yellow[2], yellow[3])
				...
				elseif job_call[playername] == 1 then
					...
					if isPointInCircle3D(x,y,z, job_pos[playername][1],job_pos[playername][2],job_pos[playername][3], 5) and result[1] and getPedWeapon(playerid) == weapon[38][2] then
						if result[1]["warehouse"] < 1000 and result[1]["money"] >= result[1]["price"] and result[1]["nalog"] ~= 0 then
	...
	if isElement ( playerid ) then
		if robbery_player[playername] == 1 then
			...
			if isPointInCircle3D(x1,y1,z1, x,y,z, radius) then
				...
				save_player_action(playername, "[robbery] "..playername.." ["..text.."], [+"..cash.."$, "..array_player_2[playername][1].."$]")
			else
...
function player_hotel (playerid, id)
	...
	if ((price_hotel) <= array_player_2[playername][1]) then
		...
		save_player_action(playername, "[player_hotel] "..playername.." [-"..(price_hotel).."$, "..array_player_2[playername][1].."$]")
		...
		if (id == v[1]) then
			...
			if (randomize2 <= v[2][randomize1][3]) then
				...
				if (inv_player_empty(playerid, id1, id2)) then
					...
					save_player_action(playername, "[random_sub] "..playername.." ["..info_png[id1][1]..", "..id2.."]")
				end
			end
			break
		end
	...
	if getElementModel(pickup) == business_icon then
		for k,v in pairs(sqlite( "SELECT * FROM business_db" )) do 
			if isPointInCircle3D(v["x"],v["y"],v["z"], x,y,z, house_bussiness_radius) then
				...
				if s_sql then
					sendPlayerMessage(playerid, "Владелец бизнеса "..s_sql, yellow[1], yellow[2], yellow[3])
				else
	...
	elseif getElementModel(pickup) == house_icon then
		for k,v in pairs(sqlite( "SELECT * FROM house_db" )) do
			if isPointInCircle3D(v["x"],v["y"],v["z"], x,y,z, house_bussiness_radius) then
				...
				if s_sql then
					sendPlayerMessage(playerid, "Владелец дома "..s_sql, yellow[1], yellow[2], yellow[3])
				else
...
function sqlite_load(playerid)
	...
	if result[1] then
		local farms = {
			[1] = {result[1]["number"], "Зарплата", result[1]["price"].."$"},
			[2] = {result[1]["number"], "Баланс", result[1]["money"].."$"},
			[3] = {result[1]["number"], "Доход от продаж", result[1]["coef"].." процентов"},
			[4] = {result[1]["number"], "Налог", result[1]["nalog"].." дней"},
			[5] = {result[1]["number"], "Склад", result[1]["warehouse"].." тушек"},
		}
...
function auction_buy_sell(playerid, value, i, id1, id2, money)--продажа покупка вещей
	...
	if value == "sell" then
		if inv_player_delet(playerid, id1, id2) then
			...
			save_player_action(playername, "[auction_sell] "..playername.." [i - "..randomize..", "..info_png[id1][1]..", "..id2..", "..money.."$]")
		else
				...
				if inv_player_empty(playerid, result[1]["id1"], result[1]["id2"]) then
					...
					for i,playerid in pairs(getElementsByType("player")) do
						...
						if playername_sell == result[1]["name_sell"] then
							sendPlayerMessage(playerid, playername.." купил у вас "..info_png[result[1]["id1"]][1].." "..result[1]["id2"].." "..info_png[result[1]["id1"]][2].." за "..result[1]["money"].."$", green[1], green[2], green[3])
					...
					if count == 0 then
						...
						if result_sell[1]["COUNT()"] == 1 then
							...
							save_player_action(result[1]["name_sell"], "[auction_buy] "..playername.." [i - "..i..", name - "..result[1]["name_sell"]..", "..info_png[result[1]["id1"]][1]..", "..result[1]["id2"].."], [+"..result[1]["money"].."$, "..array_player_2[result[1]["name_sell"]][1].."$]")

							sqlite( "UPDATE account SET inventory = '"..save_inv(result[1]["name_sell"], "player").."' WHERE name = '"..result[1]["name_sell"].."'")
						end
					end

					save_player_action(playername, "[auction_buy] "..playername.." [i - "..i..", name - "..result[1]["name_sell"]..", "..info_png[result[1]["id1"]][1]..", "..result[1]["id2"].."], [-"..result[1]["money"].."$, "..array_player_2[playername][1].."$]")
		...
		if result[1]["COUNT()"] == 1 then
			...
			if playername == result[1]["name_sell"] then
				...
				if inv_player_empty(playerid, result[1]["id1"], result[1]["id2"]) then
					...
					save_player_action(playername, "[auction_return] "..playername.." [i - "..i..", name - "..result[1]["name_sell"]..", "..info_png[result[1]["id1"]][1]..", "..result[1]["id2"].."]")
	...
	if value1 == "save" then
		...
		if prod <= result[1]["warehouse"] then
			...
			if cash <= array_player_2[playername][1] then
				...
				save_player_action(playername, "[addVehicleUpgrade_fun] [plate - "..plate..", upgrades - "..value.."], "..playername.." [-"..cash.."$, "..array_player_2[playername][1].."$], "..info_bisiness(number))
	...
	if value1 == "save" then
		...
		if prod <= result[1]["warehouse"] then
			...
			if cash <= array_player_2[playername][1] then
				...
				save_player_action(playername, "[removeVehicleUpgrade_fun] [plate - "..plate..", upgrades - "..value.."], "..playername.." [-"..cash.."$, "..array_player_2[playername][1].."$], "..info_bisiness(number))
	...
	if value1 == "save" then
		...
		if prod <= result[1]["warehouse"] then
			...
			if cash <= array_player_2[playername][1] then
				...
				save_player_action(playername, "[setVehiclePaintjob_fun] [plate - "..plate..", paintjob - "..text.."], "..playername.." [-"..cash.."$, "..array_player_2[playername][1].."$], "..info_bisiness(number))
	...
	if value1 == "save" then
		...
		if prod <= result[1]["warehouse"] then
			...
			if cash <= array_player_2[playername][1] then
				...
				save_player_action(playername, "[setVehicleColor_fun] [plate - "..plate..", color - "..text.."], "..playername.." [-"..cash.."$, "..array_player_2[playername][1].."$], "..info_bisiness(number))
	...
	if value1 == "save" then
		...
		if prod <= result[1]["warehouse"] then
			...
			if cash <= array_player_2[playername][1] then
				...
				save_player_action(playername, "[setVehicleHeadLightColor_fun] [plate - "..plate..", color - "..text.."], "..playername.." [-"..cash.."$, "..array_player_2[playername][1].."$], "..info_bisiness(number))
			...
			if v[1] == text then
				if v[3] <= array_player_2[playername][1] then
					if inv_player_empty(playerid, k, v[2]) then
						...
						save_player_action(playername, "[mayoralty_menu_fun] [mayoralty_shop - "..text.."], "..playername.." [-"..v[3].."$, "..array_player_2[playername][1].."$]")
					else
						sendPlayerMessage(playerid, "[ERROR] Инвентарь полон", red[1], red[2], red[3])
					end
				else
					...
					if v[1] == text then
						if cash*v[3] <= array_player_2[playername][1] then
							if inv_player_empty(playerid, k, v[4]) then
								...
								save_player_action(playername, "[buy_subject_fun] [weapon - "..text.."], "..playername.." [-"..cash*v[3].."$, "..array_player_2[playername][1].."$], "..info_bisiness(number))
							else
								sendPlayerMessage(playerid, "[ERROR] Инвентарь полон", red[1], red[2], red[3])
							end
						else
			...
			elseif value == 2 then
				...
				if cash <= array_player_2[playername][1] then
					if inv_player_empty(playerid, 27, text) then
						...
						save_player_action(playername, "[buy_subject_fun] [skin - "..text.."], "..playername.." [-"..cash.."$, "..array_player_2[playername][1].."$], "..info_bisiness(number))
					else
						sendPlayerMessage(playerid, "[ERROR] Инвентарь полон", red[1], red[2], red[3])
					end
				else
					...
					if v[1] == text then
						if cash*v[3] <= array_player_2[playername][1] then
							if inv_player_empty(playerid, k, v[2]) then
								...
								save_player_action(playername, "[buy_subject_fun] [24/7 - "..text.."], "..playername.." [-"..cash*v[3].."$, "..array_player_2[playername][1].."$], "..info_bisiness(number))
							else
								sendPlayerMessage(playerid, "[ERROR] Инвентарь полон", red[1], red[2], red[3])
							end
						else
					...
					if v[1] == text then
						if cash*v[3] <= array_player_2[playername][1] then
							if inv_player_empty(playerid, k, v[2]) then
								...
								save_player_action(playername, "[buy_subject_fun] [gas - "..text.."], "..playername.." [-"..cash*v[3].."$, "..array_player_2[playername][1].."$], "..info_bisiness(number))
							else
								sendPlayerMessage(playerid, "[ERROR] Инвентарь полон", red[1], red[2], red[3])
							end
						else
...
function till_fun( playerid, number, money, value )
	...
	if value == "withdraw" then
		...
		if money <= result[1]["money"] then
			...
			save_player_action(playername, "[till_fun_withdraw] "..playername.." [+"..money.."$, "..array_player_2[playername][1].."$], "..info_bisiness(number))
		else
	...
	elseif value == "deposit" then
		...
		if money <= array_player_2[playername][1] then
			...
			save_player_action(playername, "[till_fun_deposit] "..playername.." [-"..money.."$, "..array_player_2[playername][1].."$], "..info_bisiness(number))
		else
			sendPlayerMessage(playerid, "[ERROR] У вас недостаточно средств", red[1], red[2], red[3])
		end
				...
				if text == v[1] then
					...
					if count == len then
						if inv_player_empty(playerid, tonumber(split_sub_create[1]), tonumber(split_sub_create[2])) then
							...
							save_player_action(playername, "[craft_fun] "..playername.." craft ["..v[1].."]")
						else
							sendPlayerMessage(playerid, "[ERROR] Инвентарь полон", red[1], red[2], red[3])
						end
					else
...
function cow_farms(playerid, value, val1, val2)
	...
	if value == "buy" then
		...
		if cash*result > array_player_2[playername][1] then
			sendPlayerMessage(playerid, "[ERROR] У вас недостаточно средств, необходимо "..cash*result.."$", red[1], red[2], red[3])
			return
		end

		if inv_player_empty(playerid, 86, result) then
			...
			sendPlayerMessage(playerid, "Вы купили скотобойню за "..cash*result.."$", orange[1], orange[2], orange[3])
	...
	elseif value == "menu" then
		...
		if val1 == "Зарплата" then
			...
			sqlite( "UPDATE cow_farms_db SET price = '"..val2.."' WHERE number = '"..search_inv_player_2_parameter(playerid, 86).."'" )
		...
		elseif val1 == "Доход от продаж" then
			...
			sqlite( "UPDATE cow_farms_db SET coef = '"..val2.."' WHERE number = '"..search_inv_player_2_parameter(playerid, 86).."'" )
		...
		elseif val1 == "Баланс" then
			...
			if val2 < 1 then
				...
				if (val2*-1) <= result[1]["money"] then
					...
					sqlite( "UPDATE cow_farms_db SET money = money - '"..(val2*-1).."' WHERE number = '"..search_inv_player_2_parameter(playerid, 86).."'" )
				...
				if val2 <= array_player_2[playername][1] then
					...
					sqlite( "UPDATE cow_farms_db SET money = money + '"..val2.."' WHERE number = '"..search_inv_player_2_parameter(playerid, 86).."'" )
		...
		elseif val1 == "Налог" then
			...
			if search_inv_player(playerid, 60, 7) ~= 0 then
				if inv_player_delet(playerid, 60, 7) then
					sqlite( "UPDATE cow_farms_db SET nalog = nalog + '7' WHERE number = '"..search_inv_player_2_parameter(playerid, 86).."'")
	...
	elseif value == "job" then
		give_subject(playerid, "player", 87, val1)
	...
	elseif value == "load" then
		...
		if result[1]["warehouse"]-val1 < 0 then
			sendPlayerMessage(playerid, "[ERROR] Склад пуст", red[1], red[2], red[3])
			return false
		end
	...
	elseif value == "unload" then
		...
		sendPlayerMessage(playerid, "Вы разгрузили из т/с "..info_png[88][1].." "..val1.." шт ("..val2.."$ за 1 шт) за "..cash.."$", green[1], green[2], green[3])
...
function quitPlayer ( quitType )--дисконект игрока с сервера
	...
	if logged[playername] == 1 then
		...
		save_player_action(playername, "[quitPlayer] "..playername.." [heal - "..heal.."]")
...
function(ammo, attacker, weapon, bodypart)
		...
		if getElementType ( attacker ) == "player" then
			...
			if search_inv_player(attacker, 10, 1) == 0 then
				...
				if crimes[playername] ~= 0 then
					...
					save_player_action(playername_a, "[police_prison_kill] "..playername_a.." prison "..playername.." time "..(crimes[playername]).." [+"..(crimes[playername]*cash).."$, "..array_player_2[playername_a][1].."$]")
				end
			end
	...
	save_player_action(playername, "[onPlayerWasted] "..playername.." [ammo - "..tostring(ammo)..", attacker - "..tostring(playername_a)..", reason - "..tostring(reason)..", bodypart - "..tostring(getBodyPartName ( bodypart )).."]")
end)
...
function spawn_carparking( playerid, plate )
	...
	if count == 1 or result[1]["COUNT()"] == 0 then
		sendPlayerMessage(playerid, "[ERROR] Т/с в городе", red[1], red[2], red[3])
		return
	end
	...
	if search_inv_player(playerid, 61, 7) ~= 0 then
		if inv_player_delet(playerid, 61, 7) then
			...
			sendPlayerMessage(playerid, "Вы забрали т/с с номером "..plate, yellow[1], yellow[2], yellow[3])

			save_player_action(playername, "[spawn_carparking] "..playername.." [plate - "..plate.."]")
		end
	else
		sendPlayerMessage(playerid, "[ERROR] У вас нет "..info_png[61][1].." 7 "..info_png[61][2], red[1], red[2], red[3])
	end
end
	...
	if id >= 400 and id <= 611 then
		...
		if isPointInCircle3D(t_s_salon[1][1],t_s_salon[1][2],t_s_salon[1][3], x1,y1,z1, 5) then
			...
			if inv_player_empty(playerid, val1, val2) then
				save_player_action(playername, "[buy_vehicle] "..playername.." [plate - "..val2.."] [-"..cash_car[id][2].."$, "..(array_player_2[playername][1]-cash_car[id][2]).."$]")
			else
				sendPlayerMessage(playerid, "[ERROR] Инвентарь полон", red[1], red[2], red[3])
				return
			end
		...
		elseif isPointInCircle3D(t_s_salon[2][1],t_s_salon[2][2],t_s_salon[2][3], x1,y1,z1, 5) then
			...
			if inv_player_empty(playerid, val1, val2) then
				save_player_action(playername, "[buy_vehicle] "..playername.." [plate - "..val2.."] [-"..cash_helicopters[id][2].."$, "..(array_player_2[playername][1]-cash_helicopters[id][2]).."$]")
			else
				sendPlayerMessage(playerid, "[ERROR] Инвентарь полон", red[1], red[2], red[3])
				return
			end
		...
		elseif isPointInCircle3D(t_s_salon[3][1],t_s_salon[3][2],t_s_salon[3][3], x1,y1,z1, 5) then
			...
			if inv_player_empty(playerid, val1, val2) then
				save_player_action(playername, "[buy_vehicle] "..playername.." [plate - "..val2.."] [-"..cash_boats[id][2].."$, "..(array_player_2[playername][1]-cash_boats[id][2]).."$]")
			else
				sendPlayerMessage(playerid, "[ERROR] Инвентарь полон", red[1], red[2], red[3])
				return
			end
...
function throw_earth_server (playerid, value, id3, id1, id2, tabpanel)--выброс предмета
	...
	if value == "player" then
		for k,v in pairs(down_player_subject) do
			if isPointInCircle3D(x,y,z, v[1],v[2],v[3], v[4]) and id1 == v[5] then--получение прибыли за предметы
				...
				save_player_action(playername, "[throw_earth_job] "..playername.." [+"..id2.."$, "..array_player_2[playername][1].."$] ["..info_png[id1][1]..", "..id2.."]")
	...
	save_player_action(playername, "[throw_earth] "..playername.." [value - "..value..", x - "..x..", y - "..y..", z - "..z.."] ["..info_png[ id1 ][1]..", "..id2.."]")
end
		...
		for i,v in pairs(earth) do
			...
			if area then
				...
				if inv_player_empty(playerid, v[4], v[5]) then
					...
					save_player_action(playername, "[e_down] "..playername.." [x - "..v[1]..", y - "..v[2]..", z - "..v[3].."] ["..info_png[ v[4] ][1]..", "..v[5].."]")
...
function give_subject( playerid, value, id1, id2 )--выдача предметов игроку или авто
	...
	if value == "player" then
		...
		if inv_player_empty(playerid, id1, id2) then
			...
			save_player_action(playername, "[give_subject] "..playername.." [value - "..value.."] ["..info_png[id1][1]..", "..id2.."]")
		...
		if vehicleid then
			...
			elseif id1 == 88 then
				...
				elseif search_inv_player(playerid, 87, search_inv_player_2_parameter(playerid, 87)) == 0 then
					...
					return
				elseif not cow_farms(playerid, "load", symma_inv_car_1_parameter(vehicleid, 0), 0) then
					return
				end
			end
		...
		if count ~= 0 then
			...
			for k,v in pairs(sqlite( "SELECT * FROM business_db" )) do
				if isPointInCircle3D(v["x"],v["y"],v["z"], x,y,z, house_bussiness_radius) then
					...
					save_player_action(playername, "[delet_subject_business] "..playername.." [count - "..count.."], [+"..money.."$, "..array_player_2[playername][1].."$], "..info_bisiness(v["number"]))
					return
				end
			end
			...
			for k,v in pairs(down_car_subject) do
				if isPointInCircle3D(x,y,z, v[1],v[2],v[3], v[4]) then--места разгрузки
					if not cow_farms(playerid, "unload", count, sic2p) then
						...
						save_player_action(playername, "[delet_subject_job] "..playername.." [count - "..count..", price - "..sic2p.."], [+"..money.."$, "..array_player_2[playername][1].."$]")
					end
					return
				end
			end
...
-----------------------------------------------------------------------------------------------------------------------
		...
		elseif id1 == 59 then--налог дома
			...
			for k,v in pairs(sqlite( "SELECT * FROM house_db" )) do
				if isPointInCircle3D(v["x"],v["y"],v["z"], x,y,z, house_bussiness_radius) then
					...
					save_player_action(playername, "[use_59] "..playername.." использовал(а) "..info_png[id1][1].." "..id2.." "..info_png[id1][2].." и оплатил(а) "..v["number"].." дом")
		...
		elseif id1 == 60 then--налог бизнеса
			...
			for k,v in pairs(sqlite( "SELECT * FROM business_db" )) do
				if isPointInCircle3D(v["x"],v["y"],v["z"], x,y,z, house_bussiness_radius) then
					...
					save_player_action(playername, "[use_60] "..playername.." использовал(а) "..info_png[id1][1].." "..id2.." "..info_png[id1][2].." и оплатил(а) "..v["number"].." бизнес")
		...
		elseif id1 == 61 then--налог авто
			if vehicleid then
				...
				if result[1]["COUNT()"] == 1 then
					...
					save_player_action(playername, "[use_61] "..playername.." использовал(а) "..info_png[id1][1].." "..id2.." "..info_png[id1][2].." и оплатил(а) "..plate.." т/с")
			...
			if(vehicleid) then
				...
				if(job[playername] == 6) then
					...
					if(job_vehicleid[playername][1] == vehicleid) then
						...
						sendPlayerMessage(playerid, "[ERROR] Это не тот т/с", red[1], red[2], red[3])
						return
					end
				
				else
				
					sendPlayerMessage(playerid, "[ERROR] Вы не Угонщик", red[1], red[2], red[3])
					return
				end
				...
				if count == 0 then
					sendPlayerMessage(playerid, "[ERROR] Рядом нет нужного т/с", red[1], red[2], red[3])
					return
				end
			end

		elseif id1 == 85 then--лиц. угонщика
			if (crimes[playername] < crimes_giuseppe) then
		...
		--------------------------------------------------------------------------------------------------------------------------------
		save_player_action(playername, "[use_inv] "..playername.." [value - "..value.."] ["..info_png[id1][1]..", "..id2.."("..id_2..")]")
...
function win_roulette( playerid, cash, ratio )
	...
	save_player_action(playername, "[win_roulette] "..playername.." [+"..money.."$, "..array_player_2[playername][1].."$]")
end
	...
	if interior_job[14][1] == getElementInterior(playerid) and interior_job[14][10] == getElementDimension(playerid) or interior_job[13][1] == getElementInterior(playerid) and interior_job[13][10] == getElementDimension(playerid) then
		for k,v in pairs(roulette_game) do
			if v == id then
				...
				save_player_action(playername, "[los_roulette] "..playername.." [-"..cash.."$, "..array_player_2[playername][1].."$]")
				return
			end
		end
	else
						...
						if v["frozen"] == 0 then
							...
							if search_inv_player(playerid, 6, id) ~= 0 then
								if (player_in_car_theft(tostring(id)) ~= 0) then
									...
									sendPlayerMessage(playerid, "[ERROR] Т/с угнали", red[1], red[2], red[3])
									return
								end
								...
								save_player_action(playername, "[evacuationcar] "..playername.." [-"..cash.."$, "..array_player_2[playername][1].."$]")
							else
	...
	if player then
		...
		if id == player_name then
			...
			if isPointInCircle3D(x,y,z, x1,y1,z1, 10) then
				...
				save_player_action(playername, "[pay] "..playername.." give money "..id.." [-"..cash.."$, "..array_player_2[playername][1].."$]")
				save_player_action(id, "[pay] "..playername.." give money "..id.." [+"..cash.."$, "..array_player_2[id][1].."$]")
			else
	...
	if player then
		...
		if id == player_name then
			...
			if isPointInCircle3D(x,y,z, x1,y1,z1, 10) then
				...
				save_player_action(playername, "[police_prison] "..playername.." prison "..id.." time "..(crimes[id]).." [+"..(crimes[id]*cash).."$, "..array_player_2[playername][1].."$]")
			else
				sendPlayerMessage(playerid, "[ERROR] Игрок далеко", red[1], red[2], red[3])
			end
		else
	...
	if player then
		...
		if id == player_name then
			...
			if isPointInCircle3D(x,y,z, x1,y1,z1, 10) then
				...
				save_player_action(playername, "[lawyer] "..playername.." exit prison "..id.." [-"..(crimes[id]*cash).."$, "..array_player_2[playername][1].."$]")
	...
	if player then
		...
		if id == player_name then
			if inv_player_delet(player, 10, 1) then
				...
				save_player_action(playername, "[police_take_sub] "..playername.." ["..info_png[10][1]..", "..id.."]")
			else
		...
		if player then
			...
			if id == player_name then
				if inv_player_delet(player, rang, 1) then
					...
					save_player_action(playername, "[police_take_sub] "..playername.." ["..info_png[rang][1]..", "..id.."]")
				else
			...
			for h,v in pairs(sqlite( "SELECT * FROM house_db" )) do
				if isPointInCircle3D(v["x"],v["y"],v["z"], x,y,z, house_bussiness_radius) and getElementDimension(playerid) == 0 and getElementInterior(playerid) == 0 then
					if search_inv_player(playerid, 25, v["number"]) ~= 0 then
						...
						save_player_action(playername, "[buyinthouse] "..playername.." [id - "..id.."], [-"..(cash*id).."$, "..array_player_2[playername][1].."$]")
					else
						sendPlayerMessage(playerid, "[ERROR] У вас нет ключа от дома", red[1], red[2], red[3])
					end
